...
def _executor_input_requirement(executor_cls: BaseExecutor, config: dict) -> WorkflowInputRequirement:
    ...
    return WorkflowInputRequirement(kind="any", label="Any input")


def _workflow_node_schema_context(workflow_node: WorkflowNode) -> dict:
...
def render_workflow_node(request:HttpRequest) -> HttpResponse:
    ...
    schema_context = _workflow_node_schema_context(workflow_node)
    ...
    return render(
        ...
        {
            "form": selected_sub_type.executor_cls.get_config_form(
                initial=(config or {}).get("parameters", {})
            ),
            "node_type": node_type,
